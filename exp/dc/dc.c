...
dc_compression *_dc_compression_init(unsigned char *input, unsigned char *output, unsigned char thr, int verbose, int version) {
...
void _dc_get_avg_v1(dc_compression *cp, dc_corners *corp, dc_pixel *pp) {
...
void _dc_get_avg_v2(dc_compression *cp, dc_corners *corp, dc_pixel *pp, int channel) {
    int denom = (corp->x1 - corp->x0) * (corp->y1 - corp->y0);
    double val = 0.;
    int w = cp->pixel_matrix->width;
    dc_pixel *pix = cp->pixel_matrix->pixels;
    for (size_t y = corp->y0; y < corp->y1; y++) {
        for (size_t x = corp->x0; x < corp->x1; x++) {
            val += _dc_get_channel(&pix[ix(x, y, w)], channel);
        }
    }
    val /= denom;
    _dc_set_channel(pp, channel, val);
}

int _dc_get_delta_v1(dc_compression *cp, dc_corners *corp) {
    dc_pixel max = {0, 0, 0};
...
int _dc_get_delta_v2(dc_compression *cp, dc_corners *corp, int channel) {
    int max = 0;
    int min = 255;
    int w = cp->pixel_matrix->width;
    dc_pixel *pix = cp->pixel_matrix->pixels;
    for (size_t y = corp->y0; y < corp->y1; y++) {
        for (size_t x = corp->x0; x < corp->x1; x++) {
            dc_pixel *cur = &pix[ix(x, y, w)];
            int cur_channel = _dc_get_channel(cur, channel);
...
void _dc_sectorize_v1(dc_compression *cp, dc_corners cor, int splits) {
    int delta = _dc_get_delta_v1(cp, &cor);
    ...
    if (area > 1 && delta > cp->thr) {
        ...
        if (dx > dy) {
            ...
            _dc_sectorize_v1(cp, temp, splits);
        ...
        _dc_get_avg_v1(cp, &cor, &(hp->pix));
        hp->corners = cor;
        hp->splits = splits;
    }
}

void _dc_sectorize_v2(dc_compression *cp, dc_corners cor, int splits, int channel) {
    int delta = _dc_get_delta_v2(cp, &cor, channel);
    int dx = cor.x1 - cor.x0;
    int dy = cor.y1 - cor.y0;
    int area = dx * dy;
    int mid = 0;
    ...
    if (area > 1 && delta > cp->thr) {
        ...
        if (dx > dy) {
            mid = _dc_get_mid(cor.x0, cor.x1);
            temp = (dc_corners){cor.x0, cor.y0, mid, cor.y1};
            _dc_sectorize_v2(cp, temp, splits, channel);
            temp = (dc_corners){mid, cor.y0, cor.x1, cor.y1};
            _dc_sectorize_v2(cp, temp, splits, channel);
        } else {
            mid = _dc_get_mid(cor.y0, cor.y1);
        ...
        dc_heavy_sector *hp = cp->h_sectors[ix(cor.x0, cor.y0, cp->pixel_matrix->width)];
        if (hp == NULL) {
            hp = malloc(sizeof(dc_heavy_sector));
        }
        _dc_get_avg_v2(cp, &cor, &(hp->pix), channel);
        hp->corners = cor;
        hp->splits = splits;
    }
}
...
void dc_compress(unsigned char *input, unsigned char *output, unsigned char thr, int verbose, int version) {
    ...
    switch (cp->version) {
        case 1:
            _dc_sectorize_v1(cp, (dc_corners){0, 0, cp->pixel_matrix->width, cp->pixel_matrix->height}, 0);
            if (verbose) for (size_t i = 0; i < cp->pixel_matrix->width * cp->pixel_matrix->height; i++) count += cp->h_sectors[i] ? 1 : 0;
            break;
        case 2:
            _dc_sectorize_v2(cp, (dc_corners){0, 0, cp->pixel_matrix->width, cp->pixel_matrix->height}, 0, 0);
            _dc_sectorize_v2(cp, (dc_corners){0, 0, cp->pixel_matrix->width, cp->pixel_matrix->height}, 0, 1);
            _dc_sectorize_v2(cp, (dc_corners){0, 0, cp->pixel_matrix->width, cp->pixel_matrix->height}, 0, 2);
    }
